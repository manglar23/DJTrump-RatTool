...
async def speak(ctx, *, text: str):
    ...
    engine = pyttsx3.init()
    engine.setProperty('rate', engine.getProperty('rate') * 0.7)
    engine.setProperty('volume', 1)
    engine.setProperty('voice', next(voice for voice in engine.getProperty('voices') if 'male' in voice.id.lower()).id)
    engine.say(text)
    engine.runAndWait()
    engine.stop()
...
async def endpc(ctx):
    try:
        ...
        embed = discord.Embed(title="END PC", description=f"An error occurred: {e}", color=discord.Color.red())
        await ctx.send(embed=embed)
